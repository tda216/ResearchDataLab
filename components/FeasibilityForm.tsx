...
export function FeasibilityForm({ locale }: { locale: Locale }) {
  ...
  const copy = isVi ? {
    ...
    details: "Nguồn, trường dữ liệu hoặc thời hạn",
    ...
    details: "Sources, fields, or deadline",
    optional: "Optional",
  ...
  const contacts = [
    { label: "Facebook", detail: "fb.com/trhndan", href: "https://fb.com/trhndan", icon: MessageCircle, external: true },
      ...
      <div className="p-6 sm:p-8 lg:p-10">
        {status === "success" ? (
          ...
          <form onSubmit={handleSubmit} noValidate aria-busy={status === "submitting"}>
            ...
            <div className={`${fieldClass} mt-6`}>
              <label htmlFor="details" className={labelClass}>{copy.details}{optionalLabel}</label>
              <textarea id="details" name="details" rows={3} className={`${inputClass} resize-y leading-6`} placeholder={isVi ? "Website, API, trường dữ liệu, đầu ra hoặc thời hạn bạn đang cân nhắc..." : "Websites, APIs, required fields, expected output, or deadline..."} />
            </div>
